...
function getInventorybyCategory(product_category, skip,limit, callback) {
		Inventory.find({"product_category" : product_category},{skip: skip, limit: limit}, function(err, res) {
                    // In case of any error, return using the done method.
                    if (err) {         
                        return (err);
                    } else {
...
function getInventoryByCatandSort(c, s,l, callback) {
		...
		Inventory.find({product_category: c},{skip: s, limit: l}, function(err, res) {
                    // In case of any error, return using the done method.
                    if (err) {         
                        return (err);
                    } else {
...
function getOneandUpdateInventory(id, product_name, product_description, product_category, product_price ,stock_number, delivery_date,total_stock, item_supplier, supplier_name, supplier_email, supplier_number,is_available,  callback){
...
router.post('/insert',requireAuth, function(req, res) {
                var user = req.user.username;
                console.log("Username :", user);
                console.log("Req bodssy", req.body);
				...
				newInventory.save(function(err) {
									...
									console.log('New Inventory Save Succesffully');    
									//return (null, newJob);
...
router.post('/update/:id',requireAuth, function(req, res) {
                ...
                getOneandUpdateInventory(id, req.body.product_name, req.body.product_description, req.body.product_category, req.body.product_price ,req.body.stock_number, req.body.delivery_date,req.body.total_stock, req.body.item_supplier, req.body.supplier_name, req.body.supplier_email, req.body.supplier_number,req.body.is_available, function(err, result){
                    if (err){
                        res.json({"success" : false , "message" : "An Error Occured "})
                    }else{
...
router.post('/receiving/insert',requireAuth, function(req, res) {
				...
				newReceiving.additional_cost_acquisition = req.body.additional_cost_acquisition;
				...
				newReceiving.save(function(err) {
									...
									console.log('New Receiving Save Succesffully');    
									//return (null, newJob);
...
router.post('/orders/insert',requireAuth, function(req, res) {
				...
				newOrder.save(function(err) {
									...
									console.log('New Orders Save Succesffully');    
									//return (null, newJob);
