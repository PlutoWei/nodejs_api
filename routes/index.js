	...
	rCaptcha.findOne({_id : captcha_id}, function (err, captcha){
		if (err){
			...
			console.log("Yahoo captcha is", captcha);
					if(!req.body.email || !req.body.password || lowerCase != captcha.captcha) {
						if (lowerCase != captcha.captcha){
							res.json({ success: false, message: 'Invalid Captcha' });
						}else{
						
							res.json({ success: false, message: 'Please enter email and password or Invalid Captcha.' });
						}
					} else {
  ...
  router.post('/authenticate', function(req, res) {
    ...
    User.findOne({email: req.body.email}, function(err, user) {
      ...
      if (!user) {
		...
		res.status(200).json({ success: true, token: 'jwt ' + token , profile : profile});
		} else {
